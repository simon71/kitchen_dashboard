...
url = "http://www.bbc.co.uk/weather/en/2644037/?day=1"
page = urllib.request.urlopen(url)
soup = BeautifulSoup(page, "html5lib")
#table = soup.find("table", {'class':'weather moving-window three-hourly-0 one-hourly-21'})
...
#OrderedDict(sorted(item.iteritems(), key=lambda (k, v): sort_order.index(k)))
# dictionary where the key is the hour, and the value is the name of the image
# representng the weather condition
...
weather_data_tomorrow_post= {}
for i in hrs:
    if i > "05:00":
        weather_data_tomorrow_post[i] = weather_cond[i], weather_img[i], weather_temp[i], weather_humidity[i], weather_wind_speed[i], weather_wind_direction[i]

weather_data_tomorrow_pre= {}
for i in hrs:
    if i < "06:00":
        weather_data_tomorrow_pre[i] = weather_cond[i], weather_img[i], weather_temp[i], weather_humidity[i], weather_wind_speed[i], weather_wind_direction[i]
