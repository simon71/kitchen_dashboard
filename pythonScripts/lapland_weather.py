...
url = "http://www.bbc.co.uk/weather/6296559/"
...
phplus = (datetime.utcnow() + timedelta(hours=12)).strftime('%H')+':00'#time in 12 hours used in compiling json files
...
ll_weather_data_post = {}
for i in hrs:
    if i > presentHour and i != phplus:
        ll_weather_data_post[i] = weather_cond[i], weather_img[i], weather_temp[i], weather_humidity[i], weather_wind_speed[i], weather_wind_direction[i]

ll_weather_data_post1 = {}
if len(ll_weather_data_post) < 12:
    for i in hrs:
        if i < phplus:
            ll_weather_data_post1[i] = weather_cond[i], weather_img[i], weather_temp[i], weather_humidity[i], weather_wind_speed[i], weather_wind_direction[i]


#writes the json data to file
with open('../jsonData/ll_weather_data_post1.json', 'w') as outfile:
    json.dump(ll_weather_data_post1, outfile, indent=4, sort_keys=True)

with open('../jsonData/ll_weather_data_post.json', 'w') as outfile:
    json.dump(ll_weather_data_post, outfile, indent=4, sort_keys=True)
...
with open('../jsonData/ll_sun_rise.json', 'w') as outfile:
    json.dump({"set":sun_set[-5:]}, outfile, indent=4)

with open('../jsonData/ll_sun_set.json', 'w') as outfile:
